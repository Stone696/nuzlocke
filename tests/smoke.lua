-- Headless release smoke test for Nuzlocke Rules beta.29.1.0.
...
package.preload["src.core.Strings"] = function()
  local Strings = {}
  setmetatable(Strings, { __call = function(_, text, ...)
    local args = { ... }
    if #args == 0 then return text end
    return string.format(text, table.unpack(args))
  end })
  Strings.source = function(text) return text end
  Strings.load = function() end
  return Strings
end
...
entry(mod)
check(mod.exports.__beta26.build == "beta.29.1.0", "build id")
check(mod.exports.__beta26.recompCompatAudited == "0.1.83", "audited Gen1Recomp version")
check(mod.exports.__beta26.compat.Engine.profiles["0.1.82"] ~= nil
  and mod.exports.__beta26.compat.Engine.profiles["0.1.83"] ~= nil,
  "0.1.82/0.1.83 engine profiles")
check(mod.exports.nuzlocke_compat.version == 25, "compat API version")
check(mod.exports.nuzlocke_compat.getCompatibilityReport().api == 25,
...
check(store.nuzlocke_first_rival_forgiveness_armed == nil
    and store.nuzlocke_first_rival_forgiveness_triggered == nil,
  "Rival forgiveness uses durable seen-state plus battle-local flags only")
...
local starterHistory = store.nuzlocke_history or {}
check(starterHistory[#starterHistory]
    and starterHistory[#starterHistory].name == "BLAZE",
  "Gold scripted starter history stores the completed nickname")

-- A full party routes the scripted gift into PC storage. The Nuzlocke hook
-- must still initialize, register, consume the area, and enforce naming.
store.allow_gifts = true
store.encounter_limit = true
store.caught_areas = {}
gold.save.player.map = "GOLDENROD_CITY"
gold.save.party = {
  { species = "PIDGEY" }, { species = "RATTATA" }, { species = "GEODUDE" },
  { species = "ZUBAT" }, { species = "BELLSPROUT" }, { species = "WOOPER" },
}
gold.save.boxes = { {} }
local boxedVm = { specials = { renameMon = function(mon, done, opts)
  done("BOXED")
end } }
callHook("script.command", function(_, name)
  if name == "givepoke" then
    gold.save.boxes[1][#gold.save.boxes[1] + 1] = { species = "EEVEE" }
  end
end, { generation = 2, mapId = "GOLDENROD_CITY", vm = boxedVm },
  "givepoke", { "EEVEE" }, { species = "EEVEE" })
local boxedGift = gold.save.boxes[1][1]
local boxedHistory = store.nuzlocke_history or {}
check(#gold.save.party == 6 and boxedGift and boxedGift.nickname == "BOXED",
  "Gold full-party scripted gift is found and nicknamed in PC storage")
check(boxedGift.catchLocation == "GOLDENROD_CITY"
    and store.caught_areas.GOLDENROD_CITY ~= nil,
  "Gold boxed gift receives provenance and consumes its encounter area")
check(boxedHistory[#boxedHistory]
    and boxedHistory[#boxedHistory].name == "BOXED",
  "Gold boxed gift history stores the completed nickname")

-- Static provenance is a one-battle token. A trainer battle invalidates it
-- and the next unrelated wild battle must stay ordinary.
mod.exports.__beta26.pendingStaticEncounter = { generation = 2, species = "SUDOWOODO" }
local interveningTrainer = { game = gold, wild = false,
  trainer = { class = "YOUNGSTER", name = "JOEY" },
  enemy = { species = "RATTATA" } }
emit("battle.started", { game = gold, battle = interveningTrainer })
check(mod.exports.__beta26.pendingStaticEncounter == nil
    and interveningTrainer.nuzlockeStaticEncounter ~= true,
  "trainer battle consumes stale pending static provenance without inheriting it")
local ordinaryWild = { game = gold, wild = true, enemy = { species = "RATTATA" } }
emit("battle.started", { game = gold, battle = ordinaryWild })
check(ordinaryWild.nuzlockeStaticEncounter ~= true,
  "wild battle after intervening trainer remains ordinary")
mod.exports.__beta26.pendingStaticEncounter = { generation = 2, species = "SUDOWOODO" }
local intendedStatic = { game = gold, wild = true, enemy = { species = "SUDOWOODO" } }
emit("battle.started", { game = gold, battle = intendedStatic })
check(intendedStatic.nuzlockeStaticEncounter == true
    and mod.exports.__beta26.pendingStaticEncounter == nil,
  "next wild battle still consumes pending static provenance exactly once")
